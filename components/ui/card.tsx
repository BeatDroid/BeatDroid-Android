...
const Card = React.forwardRef<ViewRef, ViewProps>(
  ({ className, ...props }, ref) => (
    <View
      ...
      pointerEvents="auto"
      className={cn(
        "rounded-lg border-2 border-border dark:border-transparent bg-card shadow-md shadow-black/2 dark:shadow-none",
        className,
      )}
      {...props}
    />
...
const CardTitle = React.forwardRef<TextRef, TextProps>(
  ({ className, ...props }, ref) => (
    <Text
      ...
      ref={ref}
      className={cn(
        "text-2xl text-card-foreground font-ui-semibold leading-none tracking-tight",
        className,
      )}
      {...props}
    />
