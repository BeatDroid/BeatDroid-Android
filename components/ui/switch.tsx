...
type ColorScheme = keyof typeof RGB_COLORS;
...
>(({ className, ...props }, ref) => {
  ...
  const schemeColors = RGB_COLORS[scheme];
  ...
  const animatedRootStyle = useAnimatedStyle(() => {
    return {
      backgroundColor: interpolateColor(
        translateX.value,
        [0, 18],
        [schemeColors.input, schemeColors.primary],
      ),
    };
  });
    ...
    >
      ...
      >
        <Animated.View style={animatedThumbStyle}>
          <SwitchPrimitives.Thumb
            className={
              "h-6 w-6 rounded-full bg-card shadow-md shadow-foreground/25 ring-0"
            }
          />
        </Animated.View>
      </SwitchPrimitives.Root>
